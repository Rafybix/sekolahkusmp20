            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header header-bottom d-flex justify-content-between align-items-center">
                        ...
                        {{-- 🔹 Tombol Hapus Berita --}}
                        <form action="{{ route('backend-berita.destroy', $berita->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berita ini?');">
                            @csrf
                            @method('DELETE')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="kategori_id" class="form-control @error('kategori_id') is-invalid @enderror">
                                            ...
                                            @foreach ($kategori as $kategoris)
                                                <option value="{{ $kategoris->id }}" {{ $berita->kategori_id == $kategoris->id ? 'selected' : '' }}>
                                                    {{ $kategoris->nama }}
                                                </option>
                                            @endforeach
                                        </select>
